...
test("provider registry resolves supported hosted providers", () => {
  assert.deepEqual(listHostedProviders().sort(), ["claude", "codex", "gemini", "gemini-cli", "qwen"].sort());
...
test("provider registry rejects unsupported provider", () => {
  assert.throws(() => resolveHostedProvider("antigravity"), /Unsupported execution provider/);
});
