  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        dbg(
          "FETCH",
          `${this.provider.toUpperCase()} -> ${url} | body=${bodyStr.length}B | connectTimeout=${FETCH_CONNECT_TIMEOUT_MS}ms`,
        );
        ...
        dbg(
          "FETCH",
          `${this.provider.toUpperCase()} x ${error.name}: ${error.message}${isConnectTimeout ? " (connect timeout)" : ""}`,
        );
